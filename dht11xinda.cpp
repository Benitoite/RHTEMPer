/*####################################################################
 FILE: dht11xinda.cpp
 VERSION: 0.4.5-modified for xinda sensor
...
int dht11::read(int pin)
{
	...
	// WRITE TO RIGHT VARS
    //  
    //  xinda sensor uses byte [3] for tenths of a degree celcius. 
    // to debug your sensor, uncomment the print statements.
    
	humidity    = bits[0];
//Serial.println(bits[0]);Serial.println(bits[1]);

	temperature = bits[2]+.1*bits[3]; 
//Serial.println(bits[2]);Serial.println(bits[3]);
...
//Serial.println(sum);
	...
	if (bits[4] != sum) {            // check the sum against the checksum
  return -1;
  }
